...
import { getAppetizers } from '../../database/db'; // 👈 นำเข้าฟังก์ชันคิวรีจากไฟล์ db.js
...
function AppetizerScreen({ onBack, onAddToCart }) {
  const db = useSQLiteContext(); // 👈 แก้ไขจุดสำคัญ: เรียก Hook อยู่บนสุดของ Component เท่านั้น!
  ...
  useEffect(() => {
    ...
    async function fetchMenu() {
      try {
        // 👈 เรียกฟังก์ชันจาก db.js แทนการเขียน SQL ตรงๆ ในหน้าจอ
        const result = await getAppetizers(db);
        if (isMounted) {
          setMenuItems(result);
        }
      } catch (error) {
